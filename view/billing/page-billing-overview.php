 ...
 */
	...
	if( !$nb_alumnus ) { //If not alumni, is student
		...
		$nb_stud_date_obj->modify('-4 month'); //This should already be set to the expiration date for registration, allow 3 more months. 
		$nb_stud_exp_soon_date = clone $nb_stud_date_obj;
		...
		if( ( in_array( $nb_student_cap, $nb_renew_arr) ) && ( $today > $nb_stud_exp_soon_date ) )
			$nb_bll_actn[] = 'course_extension'; 
			
		
		//REQUEST ACCOUNT PAYOFF
		
		if( ( !empty( $nb_opt_pymnts ) || !empty( $nb_opt_reactv ) ) && ( $today < $nb_stud_end_date  ) ){
