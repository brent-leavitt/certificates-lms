...
class NB_Student { //Not sure we need the WP_User class, but it could be helpful.
	...
	public $student_post = array(
		...
		'course_access'=> 0,				//0-no access, 1-main course, 2-main & childbirth, 3-all materials
		'certification_date'=>null, 		//'2013-06-26 00:00:00',
		'num_recertification'=> 0, 			// 0
		'certificaiton_last_update'=>null,	//'2013-06-26 00:00:00',
		'course_extensions' => 0,			// 0 
		'facebook' =>null,					//'http://www.facebook.com/user/example/'
		'twitter' =>null,					//'example'
		'instagram' =>null,					//'http://www.intagram.com/user/example/'
		'pinterest' =>null,					//'http://www.pinterest.com/user/example/'
		'student_visible' => false,			//(boolean)
		'phone_visible' => false,			//(boolean)
		'email_visible' => false,			//(boolean)
		'facebook_visible' => false,		//(boolean)
		'twitter_visible' => false,			//(boolean)
		'instagram_visible' => false,		//(boolean)
		'pinterest_visible' => false		//(boolean)
	);
	...
	public function update_student( array $sPost, $update_override = 0, $display = 1 ){
		...
		$student = new WP_User( $sPost['ID'] );//We need the current student info before we can update requested student info. 
		$updated = false;
		...
		//An array to hold anything that needs to be updated from the main user object.
		$update_student = array( 'ID' => $sid );
		
		
		
		//Prep work to store checked values marked as _visible 
		$visible_arr = array_filter( $this->student_meta_keys, function($key) {
				return strpos($key, '_visible') !== false;
		});		
			
		//Update Submitted Post Data with values that are unchecked. 		
	 	foreach( $visible_arr as $vis_val )
			$sPost[ $vis_val ] = ( empty( $sPost[ $vis_val ] ) )? 'off' : 'on';		
		...
		foreach($sPost as $sPost_key => $sPost_value){
			...
			if( !empty($sPost_value) || ( $update_override == 1 ) ){
				...
				//AND it's not the STUDENT STATUS meta value. 	
					...
					$updated = update_user_meta( $sid, $sPost_key, $sPost_value, NULL ); //Nothing to compare it shouldn't be set, just add it. 
		...
		//Check Checkboxes on Student Edtior page. 
		
		$student_metadata = get_user_meta( $sid );
	...
	//This processes the data imported from Paypal transactions on the Students Import screen. 
		...
		if( empty( $sid ) ){//if Student ID is empty, this is a new student. we don't need to define the student posts if it's an update. 
			...
			$pArr = array(9,15,18,20,34,36);
			$fArr = array(100,180,200,400);
...
/* 		if( empty( $sPost['student_phone'] ) && !empty( $ppArr['Contact_Phone_Number'] ) ){
		 ...
		 */
	 	...
	 	if( empty( $sid ) ){//if Student ID is empty, this is a new student. we don't need to define the student posts if it's an update. 
			...
			$pArr = array(/* 9,15, */18,20,34,36);
			$fArr = array(/* 100, */180,200,380,400);
