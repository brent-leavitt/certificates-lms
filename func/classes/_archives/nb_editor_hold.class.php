	...
	public function load_grades_editor(){
		...
		$gradesArr = array(
			'mc_1'=> array(
				...
				'mc_1-2' => 'Assignment 1.2: The Cochrane Review',
				'mc_1-3' => 'Assignment 1.3: Listening to Mothers',
				'mc_1-4' => 'Assignment 1.4: Labor Support',
...
\n";
	...
	public function load_test_window_editor(){
		...
		//echo 'Searching for student grades in the database';
		$asmt_map = get_option('assignment_map');
		...
		foreach($results as $result){
			...
			if( $update ){
				...
				//Perform Upgrade
				$updated_result = self::upgrade_student_grades( $student_grades, $asmt_map );
	...
	public function upgrade_student_grades( $grades, $asmt_map ){
		
		$new_g = array();
		//print_pre( $grades );
		foreach( $asmt_map->certs as $cert_key => $cert){
			$new_g[ $cert_key ] = array();
			
			foreach( $cert->courses as $course_key => $course ){
				$new_g[ $cert_key ][ $course_key ]= array();
				
				foreach($course->units as $unit_key => $unit ){
					$new_g[ $cert_key ][ $course_key ][ $unit_key ] = array();
					
					foreach($unit->assignments as $asmt_key => $asmt ){
						
						//taking the new key, and reformating to look for old keys
						$old_key = substr( $asmt_key, 3 ); //incomplete, stopped here.
						$crsInit = substr( $old_key, 0, 2 );
						
						if( strcmp( $crsInit, "da" ) == 0 ||  strcmp( $crsInit, "bp" ) == 0 ){
							$asmt_str = $crsInit. '_'. substr( $old_key, 5 ); 
						} else {
						
							$isUP = substr( $old_key, 5 );
							if( !is_numeric( $isUP ) && strcmp( substr( $isUP, 0, 2 ), "up" ) == 0 ){
								$substr = "u". substr( $old_key, 3, 1 ) ."p";
								//echo "Value of isUP: $isUP. and the strlen". strlen( $isUP ) ." <br>";
								
								$substr .= ( strlen( $isUP ) == 2 )? '' : substr( $old_key, 7);
							
							} elseif( !is_numeric( $isUP ) ){
							
								$substr =  $isUP;
								
							} else {
								$substr = substr( $old_key, 3, 1 )."-".substr( $old_key, 5 );
							}
							
							$asmt_str = $crsInit. '_'.$substr;
						}
						
						//echo "Here's the generated value for the asmtKey: $asmt_str <br>";
						
						$grade = $grades[ $unit_key ][ $asmt_str ];
						
						
						//echo "grade is: $grade. <br>";
						
						$new_g[ $cert_key ][ $course_key ][ $unit_key ][ $asmt_key ] = $grade;
