 ...
 *
 *  This was the NB_Editor_Class. Moved to main "func" folder. 
 *
 *
 *
 */
	...
	function add_admin_pages() {
		...
		add_menu_page('Students Overview', 'Students', 'edit_users', 'students',  'load_students_overview'  , 'dashicons-heart', 50 );
		add_submenu_page( 'students', 'Add New Student', 'Add New', 'edit_users', 'add_student', 'load_new_student_editor' );
		add_submenu_page( NULL, 'Email Student', 'Auto Emails', 'edit_users', 'email_student', 'load_email_student_editor' );
		add_submenu_page( 'students', 'Import Students', 'Import Trxn', 'edit_users', 'import_transaction', 'load_import_transaction_editor' ); // We may do away with this one. 
		add_submenu_page( NULL, 'Edit Student', '', 'edit_users', 'edit_student', 'load_student_editor' );
		
		//TRANSACTION Editor Pages
		add_submenu_page( 'students', 'Location Search', 'Location', 'edit_users', 'location_search', 'load_location_search' );
		...
		if( ( substr( get_bloginfo('url'), 7, 6 ) ) === 'crsdev' ){
			add_submenu_page( 'students', 'Test Window', 'Test Window', 'edit_users', 'test_window',  'load_test_window_editor' );
		}
		
		//Student Maintenance Window
		add_submenu_page( 'students', 'Maintenance Tools', 'Maintenance', 'edit_users', 'maintenance',  'load_maintenance_editor' );
		
		//Admin Messages Manager page
		add_submenu_page( NULL, 'Message Manager', '', 'edit_users', 'admin_messages',  'load_admin_messages_manager' );
		
		//ASSIGNMENTS Editor page
		add_submenu_page( NULL, 'Edit Grades', '', 'edit_users', 'edit_grades',  'load_grades_editor' );
		
		//ASSIGNMENT MAP editor
		add_submenu_page( 'edit.php?post_type=assignment', 'Assignments Map Manager', 'Map Manager', 'edit_users', 'assignment_map',  'load_assignment_map_manager' );
	...
	function load_assignment_map_manager(){
		
		nb_admin_header('Assignments Map Manager'); 
	...
	function load_new_student_editor(){
		...
		load_student_form( 'Add New Student' ); //Title is the minimum variable we need to use this function. 
	...
	function load_student_editor(){
		...
		load_student_form( $studTitle, 'add_student', $student, $message, $errors );//Load the Student Form. 
	}
	...
	function load_student_form( $studTitle, $newAction = null, WP_User $student = null, $message = null, WP_Error $errors = null ){
		...
		nb_admin_header($studTitle, $newAction); 
		...
		echo '<p><small>Location: <em>classes\nb_editor.class.php 253::load_student_form</em></small><p>';
		nb_admin_footer();
	}
	 ...
	 *
	 *
	 * Called by load_transaction_editor()
	 *
	 **/	
	...
	function load_grades_editor(){
		...
		load_grades_form( 'Edit Grades', $message );  
	...
	function load_grades_form( $pageTitle, $message = null, $newAction = null ){
		...
		nb_admin_header($pageTitle, $newAction); 
			...
			foreach( $cert->courses as $course_key => $course ){
				foreach( $course->units as $unit_key => $unit ){		
					...
					foreach($unit->assignments as $asmt_key => $asmt_obj){
						...
						$nb_asmt_admin_url = get_asmt_admin_url( $asmt_key );
						echo "<tr>
							<td>
								<label for='$asmt_key'>"; 
	...
	function load_email_student_editor(){
...
EOT;
			...
			$admin_results = update_admin_notes($admin_notes, $sid, $admin_actions );			
	...
	function load_admin_messages_manager(){
		//Prep
		global $wpdb; 	
		...
		//Display
		nb_admin_header( 'Messages Manager' ); 
	...
	function load_import_transaction_editor( ){
	//	echo "This is the student import editor!";
		...
		nb_admin_header( "Batch Import Student Transactions" );
		...
		//This actually processes the transactions and updates the database. Results of this are displayed here. 
			...
			$process_results = process_import_csv($prepArr);
...
\n";
			...
			$displayBatchReport .= batchReport($process_results);
			...
			$a = 1; 
			$fileuri = check_batch_file_exists( $fileuri, $a ); 
			$fileurl = check_batch_file_exists( $fileurl, $a ); 
	 ...
	 *			
	 *
	 * Called in load_import_transaction_editor, approx line 1026
	 *
	 *
	 **/ 
	function prepare_import_csv() {
	...
	function process_import_csv( $prepArr ) {
...
/*  	private static function log_errors( $errors ) {
		...
		$log = @fopen( $log_dir_path . 'doula_csv_import_errors.log', 'a' );
	 ...
	 * @since 1.1
		...
		Also declared over in FUNC/NB_PAGES.PHP. 
	 **/
/* 	 
	 ...
	 *
	 **/	
	function load_test_window_editor(){
		global $wpdb;
	...
	**/
	...
	function load_location_search(){
		...
		nb_admin_header( "Student/Alumni Location Search" );
		...
		<?php 
		 ...
		 if( !empty( $locals ) ){
			foreach( $locals as $local ){
				...
				$asmt = new NB_Assignment( $local->ID);
		...
		//STOPPED HERE add location_search_result_line( $sid ); for each result. 
	 ...
	 *
	 **/	
	function load_maintenance_editor(){
		global $wpdb;
		$maintain_path = ABSPATH.'/maintenance/'; //Set to open on localhost. 
	...
	function batchReport($resultsArr, $sub = false ){
		$batchReport = '';
		foreach($resultsArr as $rKey => $rVal){
			if( is_array( $rVal ) ){
				if ( $rKey === 'add_mess') $sub = true;
				$batchReport .= batchReport( $rVal, $sub );
				continue;
			}	else {
	...
	function check_batch_file_exists( $file, $i ){
		
		 if( is_file( $file ) || file_exists( $file ) || is_url_exist( $file ) ){
			...
			$i++;
			$file = check_batch_file_exists( $file, $i );
		} 
